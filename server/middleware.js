...
function verifyAdminToken(token) {
    if (USE_RS256_ADMIN) return jwt.verify(token, RSA_PUBLIC_KEY, { algorithms: ['RS256'] });
...
export const requireAuth = async (req, res, next) => {
    ...
    try {
        ...
        try {
            const [rows] = db.query(
                `SELECT id FROM admin_sessions WHERE token_hash = ? AND revoked = 0 AND expires_at > datetime('now')`,
                [tokenHash]
            );
            if (!rows[0]) {
                return res
