...
export function getActiveSigningKey() {
    try {
        const [[row]] = query("SELECT kid, private_key FROM signing_keys WHERE status = 'active' ORDER BY created_at DESC LIMIT 1");
...
export function getPublicKeyByKid(kid) {
    ...
    } catch { return RSA_PUBLIC_KEY; }
}

export function getAllJwks() {
    const keys = [];
    try {
        const [rows] = query("SELECT kid, public_key FROM signing_keys WHERE status != 'retired' ORDER BY created_at DESC");
