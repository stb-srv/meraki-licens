...
router.get('/licenses', requireAuth, asyncHandler(async (req, res) => {
    ...
    const [[{ total }]] = db.query(`SELECT COUNT(*) as total FROM licenses WHERE ${where}`, params);
...
router.post('/licenses', requireAuth, asyncHandler(async (req, res) => {
    ...
    const expiresAt = raw.expires_at || toDbDate(new Date(Date.now() + plan.expires_days * 86400000));
    ...
    try {
        db.query(
            ...
            [key, raw.type || 'FREE', raw.customer_id || null, raw.customer_name || null,
             ...
             expiresAt, JSON.stringify(modules), JSON.stringify(limits),
        ...
        if (raw.customer_id) {
            ...
            try {
                const [custRows] = db.query('SELECT id, name, email FROM customers WHERE id = ?', [raw.customer_id]);
...
router.patch('/licenses/:key/status', requireAuth, asyncHandler(async (req, res) => {
    ...
    const l = rows[0];
    db.query('UPDATE licenses SET status = ? WHERE license_key = ?', [req.body.status, req.params.key]);
...
router.patch('/licenses/:key', requireAuth, asyncHandler(async (req, res) => {
    ...
    await addAuditLog('license_updated',
        ...
        req.admin.username);
    const [updated] = db.query('SELECT * FROM licenses WHERE license_key = ?', [req.params.key]);
...
router.delete('/licenses/:key', requireAuth, asyncHandler(async (req, res) => {
    try {
        const [[lic]] = db.query('SELECT customer_id FROM licenses WHERE license_key = ?', [req.params.key]);
...
router.post('/licenses/:key/upgrade', requireAuth, asyncHandler(async (req, res) => {
    ...
    return res.json({
        success: true, message: `Lizenz auf ${new_type} upgraded. Läuft ab: ${newExpiry}`,
        license_key: key, new_type, expires_at: newExpiry
    });
}));
...
router.post('/licenses/:key/extend', requireAuth, asyncHandler(async (req, res) => {
    ...
    const base = new Date(rows[0].expires_at) > new Date() ? new Date(rows[0].expires_at) : new Date();
    ...
    await addAuditLog('license_extended', { license_key: key, extended_by_days: days, new_expiry: newExpiry, actor: req.admin?.username || 'admin' });
    return res.json({ success: true, message: `Lizenz um ${days} Tage verlängert.`, license_key: key, expires_at: newExpiry });
}));
...
router.post('/licenses/:key/transfer', requireAuth, asyncHandler(async (req, res) => {
    ...
    db.query('UPDATE licenses SET associated_domain = ? WHERE license_key = ?', [normalizeDomain(new_domain) || new_domain, key]);
    await addAuditLog('license_transferred', { license_key: key, old_domain, new_domain, actor: req.admin?.username || 'admin' });
    await fireWebhook('license.transferred', { license_key: key, old_domain, new_domain });

    return res.json({ success: true, message: `Lizenz von ${old_domain} → ${new_domain} transferiert.`, license_key: key, new_domain });
}));

router.patch('/licenses/:key/customer', requireAuth, asyncHandler(async (req, res) => {
    try {
        db.query('UPDATE licenses SET customer_id = ? WHERE license_key = ?', [req.body.customer_id || null, req.params.key]);
    ...
    for (const key of keys) {
        try {
            ...
            if (action === 'renew') {
                ...
                db.query("UPDATE licenses SET expires_at = ?, status = 'active', expiry_notified_at = NULL WHERE license_key = ?", [newExpiry, key]);
                ...
                db.query('UPDATE licenses SET status = ? WHERE license_key = ?', [action === 'revoke' ? 'revoked' : 'suspended', key]);
                if (l.customer_id) {
                    db.query('DELETE FROM customer_sessions WHERE customer_id = ?', [l.customer_id]);
                ...
                db.query('UPDATE licenses SET status = ? WHERE license_key = ?', ['active', key]);
                ...
                db.query('UPDATE licenses SET customer_id = ? WHERE license_key = ?', [customer_id, key]);
...
router.get('/devices', requireAuth, asyncHandler(async (req, res) => {
    ...
    let where = '1=1';
    const params = [];
    if (license_key) { where += ' AND license_key = ?'; params.push(license_key); }
    if (search) {
    ...
    const [[{ total }]] = db.query(`SELECT COUNT(*) as total FROM devices WHERE ${where}`, params);
    const [devices]     = db.query(
        `SELECT * FROM devices WHERE ${where} ORDER BY last_seen DESC LIMIT ? OFFSET ?`,
        [...params, limit, offset]
    );
    res.json({ devices, pagination: { page, limit, total: parseInt(total), pages: Math.ceil(total / limit) } });
}));
...
router.patch('/devices/:id/deactivate', requireAuth, asyncHandler(async (req, res) => {
    try {
        ...
        if (!rows[0]) return res.status(404).json({ success: false });
        db.query(`UPDATE devices SET active = 0, deactivated_at = datetime('now') WHERE id = ?`, [req.params.id]);
...
router.post('/resellers', requireAuth, asyncHandler(async (req, res) => {
    ...
    db.query('INSERT INTO reseller_keys (api_key, name, email, max_trials, notes) VALUES (?,?,?,?,?)',
