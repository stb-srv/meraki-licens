...
router.post('/trial', requireResellerKey, asyncHandler(async (req, res) => {
    ...
    db.query('UPDATE reseller_keys SET used_trials=used_trials+1 WHERE id=?', [req.reseller.id]);

    await addAuditLog('reseller_trial_issued', { license_key: key, domain, reseller: req.reseller.name, contact_email });
    await fireWebhook('trial.registered', { license_key: key, domain, restaurant_name, contact_email, source: `reseller:${req.reseller.name}` });
    ...
    if (contact_email) {
        try {
            await sendTemplateMail('trialWelcome', contact_email, {
                ...
                plan_label: plan.label, limits: { max_dishes: plan.menu_items, max_tables: plan.max_tables }
            });
        } catch (e) { console.warn('Reseller Welcome-Mail fehlgeschlagen:', e.message); }
    }

    return res.status(201).json({
        success: true, license_key: key, expires_at: expiresAt, reseller: req.reseller.name,
