export function up(db) {
    ...
    const plans = [
        ['FREE',       'Free',       'Dauerhaft kostenlos – ideal zum Kennenlernen des Systems.', 0,   'EUR', JSON.stringify(['30 Gerichte im Menü','5 Tische','Telefonische Reservierungen','Grundlegendes Menü-System','Community Support']), 1, 0],
        ['STARTER',    'Basic',      'Für kleine Betriebe mit Wachstumsambitionen.',             29,  'EUR', JSON.stringify(['60 Gerichte im Menü','10 Tische','Mehrsprachiges Menü (DE/EN)','Küchen-Bestellsystem','Telefonische Reservierungen','E-Mail Support']), 1, 1],
        ['PRO',        'Pro',        'Voller Funktionsumfang für wachsende Restaurants.',        59,  'EUR', JSON.stringify(['150 Gerichte im Menü','25 Tische','Online-Reservierungen','5 Menü-Sprachen','QR-Pay Integration','Eigenes Branding','Küchen-Display','Prioritäts-Support']), 1, 2],
        ['ENTERPRISE', 'Enterprise', 'Maximale Leistung für große Betriebe und Ketten.',        199, 'EUR', JSON.stringify(['Unbegrenzte Gerichte','Unbegrenzte Tische','Alle Pro-Features','Analytics Dashboard','Saisonale Speisekarten','Dedicated Support','Individuelle Anpassungen']), 1, 3],
        ['TRIAL',      'Trial',      '30 Tage kostenlos alle Funktionen testen.',                0,   'EUR', JSON.stringify(['Alle Pro-Features','30 Tage gültig','Keine Kreditkarte nötig']), 0, 4],
        ['PRO_PLUS',   'Pro+',       'Erweiterte Kapazitäten und Analytics.',                   89,  'EUR', JSON.stringify(['300 Gerichte','50 Tische','Analytics Dashboard','Alle Pro-Features']), 0, 5],
    ];
    for (const [plan_id, label, description, price, currency, features, active, sort_order] of plans) {
        db.prepare('INSERT OR IGNORE INTO plan_pricing (plan_id,label,description,price,currency,features,active,sort_order) VALUES (?,?,?,?,?,?,?,?)').run(plan_id, label, description, price, currency, features, active, sort_order);
    }

    // Standard-FAQ seeden
    const faqs = [
        ['faq-1', 'Was bedeutet "Bis zu X Tische"?',
         ...
         'Lizenz & Pläne', 0],
        ['faq-2', 'Was bedeutet "X Gerichte im Menü"?',
        ...
        ['faq-4', 'Kann ich meinen Plan jederzeit wechseln?',
         ...
         'Lizenz & Pläne', 3],
        ['faq-5', 'Was passiert wenn meine Lizenz abläuft?',
        ...
        ['faq-6', 'Wie wird abgerechnet?',
         ...
         'Abrechnung', 5],
        ['faq-7', 'Benötige ich eine eigene Domain?',
         ...
         'Technik', 6],
        ['faq-8', 'Was ist das Kunden-Portal?',
    ...
    for (const [id, question, answer, category, sort_order] of faqs) {
        db.prepare('INSERT OR IGNORE INTO faq (id,question,answer,category,sort_order) VALUES (?,?,?,?,?)').run(id, question, answer, category, sort_order);
    }
}
