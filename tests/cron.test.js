...
describe('runOverdueInvoiceCron dunning logic', () => {
    ...
    test('promotes invoice to dunning level 1 when 1 day overdue', async () => {
        ...
        jest.spyOn(db, 'query').mockImplementation((sql, params) => {
            ...
            if (sql.includes('FROM invoices i')) {
                return [[{
                    id: 'inv1', invoice_number: 'INV-2024-0001',
                    customer_id: 'cust1', amount_gross: 29.00,
                    due_date: '2024-01-01', dunning_level: 0,
                    days_overdue: 1, email: 'test@example.com',
        ...
        const dunningUpdate = queryCalls.find(c =>
            c.sql.includes('UPDATE invoices') && c.sql.includes('dunning_level')
        );
    ...
    test('promotes to level 2 when 7+ days overdue', async () => {
        ...
        jest.spyOn(db, 'query').mockImplementation((sql, params) => {
            ...
            if (sql.includes('FROM invoices i')) {
                return [[{
                    id: 'inv2', invoice_number: 'INV-2024-0002',
                    customer_id: 'cust1', amount_gross: 59.00,
                    due_date: '2024-01-01', dunning_level: 1,
                    days_overdue: 8, email: 'test@example.com',
                    customer_name: 'Test Kunde'
                }]];
            }
            return [{ affectedRows: 1, insertId: 0 }];
    ...
    test('suspends license at dunning level 4 (30+ days overdue)', async () => {
        ...
        jest.spyOn(db, 'query').mockImplementation((sql, params) => {
            ...
            if (sql.includes('FROM invoices i')) {
                return [[{
                    id: 'inv3', invoice_number: 'INV-2024-0003',
                    customer_id: 'cust99', amount_gross: 199.00,
                    due_date: '2024-01-01', dunning_level: 3,
                    days_overdue: 31, email: 'overdue@example.com',
        ...
        const suspend = queryCalls.find(c =>
            c.sql.includes('UPDATE licenses') && c.sql.includes('suspended')
        );
    ...
    test('skips invoice already at target dunning level (idempotent)', async () => {
        ...
        jest.spyOn(db, 'query').mockImplementation((sql, params) => {
            ...
            if (sql.includes('FROM invoices i')) {
                return [[{
                    id: 'inv4', invoice_number: 'INV-2024-0004',
                    customer_id: 'cust2', amount_gross: 29.00,
                    due_date: '2024-01-01', dunning_level: 1,
                    days_overdue: 3, email: 'test@example.com',
                    customer_name: 'Test'
                }]];
            }
...
describe('runExpiryCron configurable intervals', () => {
    ...
    test('marks licenses as expired when past expiry date', async () => {
        ...
        jest.spyOn(db, 'query').mockImplementation((sql, params) => {
            queryCalls.push({ sql, params });
            if (sql.includes('expiry_warn_days_1')) return [[{ expiry_warn_days_1: 30, expiry_warn_days_2: 7 }]];
